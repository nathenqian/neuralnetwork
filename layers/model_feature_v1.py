...
#        name = "euc"
#    )
    softmax = ColumnWiseSoftmax(name = 'softmax')
    ...
    fc_out = fc2.fprop(multimodal_image_out2)

    my_out = softmax.fprop(fc_out)
    my_cost = softmax.cost(fc_out, y_truth)
...
def readDataFile(base_dir):
    fin = open(os.path.join(base_dir, 'config_all.conf'), 'r')
    data = loads(fin.read())
    fin.close()
    dt = {}
    for item in data:
        for stc in item['sentence']:
            for word in stc.split(' '):
                if len(word) == 0:
                    continue
                flag = True
                for cc in word:
                    if not cc in english_char:
                        flag = False
                        break
                if not flag:
                    continue
    ...
    for item in dt:
        if dt[item] < 50:
            word_dictionary[item]=t_no
            t_no+=1

#    from IPython import embed;embed()

    data_generator = DataGeneratorFeature(os.path.join(base_dir, "config_all.conf"), word_dictionary, "../local_data/image_npy")
...
#for test
    run_func = theano.function(inputs=[label], outputs=[output], on_unused_input='warn', allow_input_downcast=True)
    _dir = '../local_data/image_npy'
#    for ii in os.listdir(_dir):
    ii = '20264.config.npy'
    print ii
    img = np.load(os.path.join(_dir, ii))
    myin = np.zeros((1,1,4096,1), dtype='float32')
    myin[0,0,:,0] = img
    myout = run_func(myin) 
    embed()
#end
